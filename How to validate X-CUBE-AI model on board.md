...
2. When generating code, make sure to select `Basic` option in `Application Structure`. In order to operate the header file later, don't forget to check option `Generate peripheral initialization as a pair of '.c/.h' files per peripheral` in the Code Generator when generating code.
